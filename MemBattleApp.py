...
class MyGridT(Widget):  #Log in
    ...
    def DBrecieve(self,User, Pass):
        mycursor = requests.get("http://Mega:8100/login").json()
        userL =  []
        passL = []
        UserID_L = []
        for i in mycursor:
...
class ForgotPass(Widget):
    ...
    def newsend(self,User,Scrt,Pass):
        mycursor = requests.get("http://Mega:8100/scrt").json()
        userL = []
        scrtL = []
        for i in mycursor:
            userL.append(i[0])
        ...
        if User.text not in userL:
            ...
            if md5(Scrt.text) == scrtL[ind]:
                data = {'password': md5(Pass.text),
                        'user': User.text}
                headers = {'content-type': 'application/json'}
                r = requests.post("http://Mega:8100/scrt", data=json.dumps(data), headers=headers)
...
class SellectionGrid(Widget):
    difficulty = ""
    difficultyBase = ""   #tega ne dajem na nič ker rabim za pošiljanje v bazo, vrednost bo spremenilo pri novi igri
    nop = ""
    nopBase = ""          #tega ne dajem na nič ker rabim za pošiljanje v bazo, vrednost bo spremenilo pri novi igri
    difficultyBaselist = []   #zaradi težavnsti prenosa(v drugi class) uporabljam listo
    nopBaselist = []
...
class Pop4(FloatLayout):
    def show_popup(self):
        show = Pop4()
        popupWindow = Popup(title="Note", content=show, size_hint=(None, None), size=(190, 100))
        popupWindow.open()
    def switch(self,*args):
...
class Pop5(FloatLayout):
    def show_popup(self):
        show = Pop5()
        popupWindow = Popup(title="Note", content=show, size_hint=(None, None), size=(225, 105))
        popupWindow.open()
...
class Pop7(FloatLayout):
    def show_popup(self):
        show = Pop7()
        popupWindow = Popup(title="Note", content=show, size_hint=(None, None), size=(210, 110))
        popupWindow.open()

class Pop8(FloatLayout):
    def show_popup(self):
        show = Pop8()
        popupWindow = Popup(title="Note", content=show, size_hint=(None, None), size=(235, 100))
        popupWindow.open()

class Pop9(FloatLayout):
    def show_popup(self):
        show = Pop9()
        popupWindow = Popup(title="Note", content=show, size_hint=(None, None), size=(190, 110))
        popupWindow.open()
...
class PuzzleWindow(Screen):
    ...
    def on_enter(self):
        ...
        for i in range(len(self.ids.puzz.ids.puzzG.children[:])):
            ...
            if self.check_press:
                self.ids.puzz.ids.puzzG.children[i].bind(on_press=lambda x, child=self.ids.puzz.ids.puzzG.children[i], st=i: self.newspress(child, st))
        self.score=0
    ...
    def clockturn(self, *args):
        self.ids.puzz.ids.puzzG.children[self.index_list[1]].a = "C:/Users/Maj/Desktop/markF.jpg"
        self.ids.puzz.ids.puzzG.children[self.index_list[0]].a = "C:/Users/Maj/Desktop/markF.jpg"
        self.pict_list = []
        self.index_list = []
    def clockcorrect(self, *args):
        ...
        self.pict_list = []
        self.index_list = []
    def clockcorrectdis(self, *args):   #da ne dobim errorja za igralce, ki radi hitro igrajo
        self.score -= 0.5
        self.ids.puzz.ids.puzzS.children[0].text = str(self.score)
        self.ids.puzz.ids.done.children[0].disabled = True
        for i in range(len(self.ids.puzz.ids.puzzG.children[:])):
            self.ids.puzz.ids.puzzG.children[i].disabled = True
    def clockcorrectenable(self, *args):  # da ne dobim errorja za igralce, ki radi hitro igrajo
        for i in range(len(self.ids.puzz.ids.puzzG.children[:])):
            self.ids.puzz.ids.puzzG.children[i].disabled = False
            self.ids.puzz.ids.done.children[0].disabled = False   #to je za to da ne grem pre hitro nazaj, v kolikor to storim game crasha
    def clockcorrectdisable(self, *args):  # da ne dobim errorja za igralce, ki radi hitro igrajo
...
class PuzzleWindowComp(Screen):
    ...
    def on_enter(self):
        ...
        for i in range(len(self.ids.puzzc.ids.puzzGC.children[:])):
            ...
            if self.check_press:
                self.ids.puzzc.ids.puzzGC.children[i].bind(on_press=lambda x, child=self.ids.puzzc.ids.puzzGC.children[i], st=i: self.newspress(child, st))
        self.score = 0
    ...
    def newspress(self, child, i):
        ...
        try:
            ...
            pass
    def clockturn(self, *args):
        self.ids.puzzc.ids.puzzGC.children[self.index_list[1]].a = "C:/Users/Maj/Desktop/markF.jpg"
        self.ids.puzzc.ids.puzzGC.children[self.index_list[0]].a = "C:/Users/Maj/Desktop/markF.jpg"
        self.pict_list = []
        self.index_list = []
    def clockcorrect(self, *args):
        self.score += 1
...
class PuzzleWindowComp4(Screen):
    ...
    def on_enter(self):
        ...
        for i in range(len(self.ids.puzzc4.ids.puzzGC4.children[:])):
            ...
            if self.check_press:
                self.ids.puzzc4.ids.puzzGC4.children[i].bind(on_press=lambda x, child=self.ids.puzzc4.ids.puzzGC4.children[i], st=i: self.newspress(child, st))
        self.score = 0
        Clock.schedule_once(self.clocka, 2)
    def on_leave(self):
        for i in range(len(self.ids.puzzc4.ids.puzzGC4.children[:])):
    ...
    def newspress(self, child, i):
        ...
        try:
            ...
            pass
    def clockturn(self, *args):
        self.ids.puzzc4.ids.puzzGC4.children[self.index_list[1]].a = "C:/Users/Maj/Desktop/markF.jpg"
        self.ids.puzzc4.ids.puzzGC4.children[self.index_list[0]].a = "C:/Users/Maj/Desktop/markF.jpg"
        self.pict_list = []
        self.index_list = []
    def clockcorrect(self, *args):
        self.score += 1
        self.ids.puzzc4.ids.puzzSC4.children[0].text = str(self.score)
        self.ids.puzzc4.ids.puzzGC4.children[self.index_list[1]].disabled = True
        self.ids.puzzc4.ids.puzzGC4.children[self.index_list[0]].disabled = True
        self.pict_list = []
        self.index_list = []
    def clockcorrectdis(self, *args):   #da ne dobim errorja za igralce, ki radi hitro igrajo
        self.score -= 0.5
    ...
    def finishDir(self):
        ...
        if self.class_instance[0][0] == "2":
            if self.countswich == 0:
                ...
                data['gamescore'] = userL[userIndex] + ': ' + self.gameScore_list[0] + '  ' + 'P2: ' + self.gameScore_list[1]
